...
fl = open('D:\\Code\\qzone_analyse\\example\\newest_example1\\rough.json', 'r', encoding='utf-8')
rough = json.load(fl)
fl.close()
for i in range(len(rough)):
    print(i)
    fl_fine = open('D:\\Code\\qzone_analyse\\example\\newest_example1\\fine_%s.json' % i, 'r', encoding='utf-8')
    fine = json.load(fl_fine)
    fl_fine.close()
    parse_r = json_parse.rough_json_parse(rough, i)
    fl = open('D:\\Code\\qzone_analyse\\example\\newest_example1\\parse\\rough_%s.json' % i, 'w', encoding='utf-8')
    fl.write(json.dumps(parse_r, ensure_ascii=False, indent=2))
    fl.close()
    parse_f = json_parse.fine_json_parse(rough, i, fine)
    fl = open('D:\\Code\\qzone_analyse\\example\\newest_example1\\parse\\fine_%s.json' % i, 'w', encoding='utf-8')
