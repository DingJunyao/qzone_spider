...
def connect():
    conn = psycopg2.connect(database=svar.dbDatabase, user=svar.dbUsername, password=svar.dbPassword, host=svar.dbURL, port=svar.dbPort)
    logger.info('本爬虫在 %s 的 %s 数据库 %s 连接成功' % (svar.dbURL, svar.dbType, svar.dbDatabase))
    cursor = conn.cursor()
...
def db_init():
    ...
    logger.info('本爬虫在 %s 的 %s 数据库 %s 初始化完成' % (svar.dbURL, svar.dbType, svar.dbDatabase))
...
def db_write_rough(parse):
    ...
    if parse['voice'] is not None:
        ...
        try:
            insert_sql = '''
                                  INSERT INTO media("type",url,"time")
                                  VALUES (%s,%s,%s)
    ...
    try:
        ...
        cursor.execute(insert_sql, (
            datetime.datetime.fromtimestamp(parse['catch_time']), parse['tid'], parse['qq'], datetime.datetime.fromtimestamp(parse['post_time']), parse['rt_tid'],
...
def db_write_fine(parse):
    ...
    if parse['piclist'] is not None:
        ...
        for i in range(len(parse['piclist'])):
            ...
            try:
                insert_sql = '''
                              INSERT INTO media("type",url,thumb)
                              VALUES (%s,%s,%s)
                              ON CONFLICT(url) do nothing;
                    '''
                ...
                logger.info('成功将图片插入到数据库')
            except Exception as e:
                conn.rollback()
                logger.error('试图将图片存入数据库时出错')
                pass
    ...
    if parse['video'] is not None:
        ...
        try:
            insert_sql = '''
                                  INSERT INTO media("type",url,thumb,"time")
                                  VALUES (%s,%s,%s,%s)
                                  ON CONFLICT(url) do nothing;
                        '''
            cursor.execute(insert_sql,
            ...
            logger.info('成功将视频插入到数据库')
        except Exception as e:
            conn.rollback()
            logger.error('试图将视频存入数据库时出错')
