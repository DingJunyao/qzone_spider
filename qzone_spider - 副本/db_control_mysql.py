...
ALTER TABLE `media` ADD UNIQUE `url` (`url`(255));
...
def db_write_rough(parse):
    ...
    if parse['voice'] is not None:
        ...
        try:
            insert_sql = '''
                              INSERT IGNORE INTO media(type,url,time)
                              VALUES (%s,%s,%s);
                    '''
...
def db_write_fine(parse):
    ...
    if parse['piclist'] is not None:
        ...
        for i in range(len(parse['piclist'])):
            ...
            try:
                insert_sql = '''
                          INSERT IGNORE INTO media(type,url,thumb)
                          VALUES (%s,%s,%s);
                '''
    ...
    if parse['video'] is not None:
        ...
        try:
            insert_sql = '''
                              INSERT IGNORE INTO media(type,url,thumb,time)
                              VALUES (%s,%s,%s,%s);
                    '''
            cursor.execute(insert_sql, ('video', parse['video']['url'], parse['video']['thumb'], parse['video']['time']))
            conn.commit()
