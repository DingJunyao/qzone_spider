    ...
    while fail < svar.getRoughDataFailTime:
        ...
        try:
            ...
            if fail == svar.getFineDataFailTime:
                break
            # logger.warning('获取QQ为 %s 的 %s ~ %s 的说说粗略数据时连接异常，休息 %s 秒后重试，剩余重试次数为: %s' % (qq, start, start + msgnum - 1, svar.errorWaitTime, svar.getRoughDataFailTime - fail))
            logger.warning(
                'Connection error when getting the rough JSON of messages #%s ~ #%s of %s. Sleep %s seconds before retrying. Remaining retry times: %s'
                % (start, start + msgnum - 1, qq, svar.errorWaitTime, svar.getRoughDataFailTime - fail))
        ...
        if (response.status_code == 200) & (response.text is not None):
            ...
            if fail == svar.getFineDataFailTime:
                break
            # logger.warning('获取QQ为 %s 的 %s ~ %s 的说说粗略数据时请求失败，休息 %s 秒后重试，剩余重试次数为: %s' % (qq, start, start + msgnum - 1, svar.errorWaitTime, svar.getRoughDataFailTime - fail))
            logger.warning('Failed to request when getting the rough JSON of messages #%s ~ #%s of %s. Sleep %s seconds before retrying. Remaining retry times: %s'
                           % (start, start + msgnum - 1, qq, svar.errorWaitTime, svar.getRoughDataFailTime - fail))
...
def get_fine_json(qq, tid, cookie, gtk, qzonetoken):
    ...
    while fail < svar.getFineDataFailTime:
        ...
        try:
            ...
            if fail == svar.getFineDataFailTime:
                break
            # logger.warning('获取QQ为 %s 的tid为 %s 的说说JSON时连接异常，休息 %s 秒后重试，剩余重试次数为: %s' % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
            logger.warning(
                'Connection error when getting the JSON of message of %s which tid is %s. Sleep %s seconds before retrying. Remaining retry times: %s'
                % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
            time.sleep(svar.errorWaitTime)
            continue
        if (response_msg.status_code == 200) & (response_msg.text is not None):
            ...
            if response_msg_json['ret'] >= 0:
                ...
                if fail == svar.getFineDataFailTime:
                    break
                # logger.warning('获取QQ为 %s 的tid为 %s 的说说JSON时返回异常，休息 %s 秒后重试，剩余重试次数为: %s' % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
                logger.warning(
                    'Strange return when getting the JSON of message of %s which tid is %s. Sleep %s seconds before retrying. Remaining retry times: %s'
                    % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
            ...
            if fail == svar.getFineDataFailTime:
                break
            # logger.warning('获取QQ为 %s 的tid为 %s 的说说数据时请求失败，休息 %s 秒后重试，剩余重试次数为: %s' % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
            logger.warning(
                'Failed to request when getting the JSON of message of %s which tid is %s. Sleep %s seconds before retrying. Remaining retry times: %s'
                % (qq, tid, svar.errorWaitTime, svar.getFineDataFailTime - fail))
